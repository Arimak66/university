...
namespace university
{
    public class Course
    {
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"INSERT INTO  course  (idcourse,name,creditpoints) VALUES (@idcourse,@name,@creditpoints);";
            BindId(cmd);
            BindParams(cmd);
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"UPDATE  course  SET  name  = @name,  creditpoints = @creditpoints WHERE idcourse = @idcourse;";
            BindId(cmd);
            BindParams(cmd);
            await cmd.ExecuteNonQueryAsync();
        }
            ...
            using (reader)
            {
                while (await reader.ReadAsync())
                {
                    var post = new Course(Db)
                    {
                        ...
                        creditpoints = reader.GetInt32(2),
